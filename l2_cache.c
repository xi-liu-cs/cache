...
/***********************************************************
  ...
  1MB direct-mapped, write-back cache.
   ...
   per cache line, the number of cache lines is:
      ...
      (2^20 bytes)(cache line/2^5 bytes) = 2^15 cache lines
...
void l2_initialize()
{
  for(uint32_t i=0; i<L2_NUM_CACHE_ENTRIES; i++) 
  {
                     ...
                     uint8_t control, uint32_t read_data[], uint8_t *status)
{
  ...
  ( (l2_cache[index].v_d_tag & L2_ENTRY_TAG_MASK) != tag) )
  {*status = (*status) & (~1);}
  //CODE HERE
  ...
  else
  {
    if(control & READ_ENABLE_MASK)
    {
      ...
      {read_data[i] = l2_cache[index].cache_line[i];} //copy line data to read array
    }
    if(control & WRITE_ENABLE_MASK)
    {
      for(uint32_t i=0; i<WORDS_PER_CACHE_LINE; i++)
      {l2_cache[index].cache_line[i] = write_data[i];} //copy data in write array into line
      l2_cache[index].v_d_tag = l2_cache[index].v_d_tag | L2_DIRTYBIT_MASK; //d=1
    }
    *status = (*status) | 1; //low bit = 1
  }
  //CODE HERE
                    ...
                    uint8_t *status)
{
  ...
  {
    for(uint32_t i=0; i<WORDS_PER_CACHE_LINE; i++)
    {l2_cache[index].cache_line[i] = write_data[i];} //overwrite
    l2_cache[index].v_d_tag = l2_cache[index].v_d_tag | L2_VBIT_MASK; //v=1
    l2_cache[index].v_d_tag = l2_cache[index].v_d_tag & ~L2_DIRTYBIT_MASK; //d=0
    l2_cache[index].v_d_tag = (l2_cache[index].v_d_tag & ~L2_ENTRY_TAG_MASK)| tag; //write tag into entry
  ...
  else if( (l2_cache[index].v_d_tag & L2_VBIT_MASK) 
  && l2_cache[index].v_d_tag & L2_DIRTYBIT_MASK)
  {
    *evicted_writeback_address = 
    ( (l2_cache[index].v_d_tag & L2_ENTRY_TAG_MASK) << L2_ADDRESS_TAG_SHIFT) 
    | (index << L2_INDEX_SHIFT);
    for(uint32_t i=0; i<WORDS_PER_CACHE_LINE; i++)
