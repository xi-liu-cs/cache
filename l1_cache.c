...
void l1_initialize()
{
  for(uint32_t i=0; i<512; i++) //512 sets
  {
    for(uint32_t j=0; j<4; j++) //4 lines per set
                     ...
                     uint8_t control, uint32_t *read_data, uint8_t *status)
{
  ...
  uint32_t index = (address & L1_SET_INDEX_MASK) >> L1_SET_INDEX_SHIFT;
  //CODE HERE
  ...
  uint32_t woffset = (address & WORD_OFFSET_MASK) >> WORD_OFFSET_SHIFT;
  //CODE HERE
  ...
  uint32_t hit = 0;
  for(uint32_t i=0; i<4; i++) //4 lines per set
  {
    if( (l1_cache[index].lines[i].v_r_d_tag & L1_VBIT_MASK)
     && ( (l1_cache[index].lines[i].v_r_d_tag & L1_ENTRY_TAG_MASK) == tag) )
    {
      ...
      *status = *status | 1;
      ...
      {
        *read_data = l1_cache[index].lines[i].cache_line[woffset];
      }
      if(control & WRITE_ENABLE_MASK)
      {
        l1_cache[index].lines[i].cache_line[woffset] = write_data;
                    ...
                    uint8_t *status)
{
  ...
  // can return
    ...
    if( !(l1_cache[index].lines[i].v_r_d_tag & L1_VBIT_MASK) ) //v=0
    {
      for(uint32_t j=0; j<WORDS_PER_CACHE_LINE; j++)
      {l1_cache[index].lines[i].cache_line[j] = write_data[j];}

      l1_cache[index].lines[i].v_r_d_tag = l1_cache[index].lines[i].v_r_d_tag
      | L1_VBIT_MASK; //v=1
      l1_cache[index].lines[i].v_r_d_tag = l1_cache[index].lines[i].v_r_d_tag
      & (~L1_DIRTYBIT_MASK); //d=0
      l1_cache[index].lines[i].v_r_d_tag = l1_cache[index].lines[i].v_r_d_tag
      & (~L1_RBIT_MASK); //r=0

      l1_cache[index].lines[i].v_r_d_tag = (l1_cache[index].lines[i].v_r_d_tag
      & ~L1_ENTRY_TAG_MASK ) | tag;

      *status = (*status) & ~1; //low bit = 0
  ...
  //  the first entry that has r=0 and d=1, etc. 
   ...
   {
      if( !~r0_d0_index &&
        ( !(l1_cache[index].lines[i].v_r_d_tag & L1_RBIT_MASK) 
      && !(l1_cache[index].lines[i].v_r_d_tag & L1_DIRTYBIT_MASK) 
      ) ) //r=0, d=0 
      {r0_d0_index = i;}
      else if(!~r0_d1_index &&
      ( !(l1_cache[index].lines[i].v_r_d_tag & L1_RBIT_MASK) 
      && (l1_cache[index].lines[i].v_r_d_tag & L1_DIRTYBIT_MASK) )) //r=0, d=1 
      {r0_d1_index = i;}
      else if(!~r1_d0_index &&
      ((l1_cache[index].lines[i].v_r_d_tag & L1_RBIT_MASK) 
      && !(l1_cache[index].lines[i].v_r_d_tag & L1_DIRTYBIT_MASK) )
  ...
  uint32_t evict = 0;
  if(r0_d0_index != (~0x0) ){evict = r0_d0_index;}
  else if(r0_d1_index != (~0x0) ){evict = r0_d1_index;}
  else if(r1_d0_index != (~0x0) ){evict = r1_d0_index;}
  //CODE HERE
  ...
  if(l1_cache[index].lines[evict].v_r_d_tag & L1_DIRTYBIT_MASK)
  {
    *evicted_writeback_address = 
    ( (l1_cache[index].lines[evict].v_r_d_tag & L1_ENTRY_TAG_MASK) << L1_ADDRESS_TAG_SHIFT) 
    | (index << L1_SET_INDEX_SHIFT);
    for(uint32_t i=0; i<WORDS_PER_CACHE_LINE; i++)
    {evicted_writeback_data[i] = l1_cache[index].lines[evict].cache_line[i];}
  }
  ...
  //the entry.
  for(uint32_t i=0; i<WORDS_PER_CACHE_LINE; i++)
  {l1_cache[index].lines[evict].cache_line[i] = write_data[i];}

  l1_cache[index].lines[evict].v_r_d_tag = 
  l1_cache[index].lines[evict].v_r_d_tag | L1_VBIT_MASK;    //v=1
  l1_cache[index].lines[evict].v_r_d_tag = 
  l1_cache[index].lines[evict].v_r_d_tag & ~L1_DIRTYBIT_MASK; //d=0
    l1_cache[index].lines[evict].v_r_d_tag = 
  l1_cache[index].lines[evict].v_r_d_tag & ~L1_RBIT_MASK; //r=0
  l1_cache[index].lines[evict].v_r_d_tag = 
  (l1_cache[index].lines[evict].v_r_d_tag & ~L1_ENTRY_TAG_MASK) | tag;
  //CODE HERE
}
